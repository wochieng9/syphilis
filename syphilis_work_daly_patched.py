...
v4 changes vs v3
────────────────
AUDIT PATCH — internal consistency fixes
- Unified PSA and deterministic outcome-count logic through one shared arm engine.
- Honoured caller-supplied baseline coverage (sc_b) in PSA/deterministic helpers.
- Propagated treatment-completion overrides through downstream tabs/helpers.
- Added serofast confirmatory-test cost to CEA and linked serofast prevalence in OWSA.
- Computed aggregate CrIs from iteration-level sums and removed duplicate dataclass/global warning suppression.
...
def beta_ab(m, lo, hi) -> Tuple[float, float]:
    """
    Convert an approximate 95% uncertainty interval into beta shape parameters.

    The UI allows users to move some means outside the original low/high PSA
    ranges. Rather than producing invalid beta parameters, this helper widens
    the interval just enough to contain the selected mean while preserving the
    original interval width whenever possible.
    """
    m = float(np.clip(m, 1e-6, 1.0 - 1e-6))
    lo = float(np.clip(lo, 1e-9, 1.0 - 1e-9))
    hi = float(np.clip(hi, 1e-9, 1.0 - 1e-9))

    if hi <= lo:
        span = max(abs(hi - lo), 1e-6)
        lo = max(1e-9, m - span / 2.0)
        hi = min(1.0 - 1e-9, m + span / 2.0)

    if not (lo < m < hi):
        span = max(hi - lo, 0.10 * min(m, 1.0 - m), 1e-5)
        lo = min(lo, max(1e-9, m - span / 2.0))
        hi = max(hi, min(1.0 - 1e-9, m + span / 2.0))
        if lo >= m:
            lo = max(1e-9, m - max(span, 1e-5))
        if hi <= m:
            hi = min(1.0 - 1e-9, m + max(span, 1e-5))
        if hi <= lo:
            lo = max(1e-9, m * 0.5)
            hi = min(1.0 - 1e-9, m + max(0.5 * (1.0 - m), 1e-5))

    var = max(((hi - lo) / 4.0) ** 2, 1e-10)
    ab = max(m * (1.0 - m) / var - 1.0, 1e-3)
    return max(m * ab, 1e-3), max((1.0 - m) * ab, 1e-3)
...
def sigmoid_ramp(
    ...
    At t_ninety, coverage reaches 90% of the gain from usual care to
    target coverage; target coverage remains the upper asymptote.
...
def bia_annual_impact(
    ...
    # Outcomes averted
    # ------------------------------------------------------------------
    # Use the same arm-count engine as the PSA/deterministic CEA. The BIA
    # denominator is the incremental screened population, so coverage and
    # pregnancy-identification effects have already been applied in the funnel.
    dlt_out = _incremental_screening_outcome_delta(
        n_screened = cas["n_screened"],
        p_act      = p_act,
        sens       = sens,
        p_adeq     = p_adeq,
        tx_eff     = tx_eff,
    ...
    n_cs_comp   = float(dlt_out["cs_comp"])
    n_cs_uncomp = float(dlt_out["cs_uncomp"])
    n_cs_av     = n_cs_comp + n_cs_uncomp
    n_pt_av     = float(dlt_out["preterm"])
    n_sb_av     = float(dlt_out["stillbirth"])
    n_nnd_av    = float(dlt_out["neonatal_death"])

    n_iufd    = float(dlt_out["iufd_subset"])
...
def _draw_serofast(
    ...
    # Keep bounds ordered even when the linked serofast prevalence is very low
    # (e.g., low-prevalence rural preset).
    p_sf = float(max(p_sf, 1e-9))
    lo_sf = max(p_sf * 0.50, 1e-6)
    hi_sf = min(max(p_sf * 1.50, lo_sf * 1.01), 0.15)
    a, b = beta_ab(p_sf, lo=lo_sf, hi=hi_sf)
    psa["p_sf"] = bd(a, b).rvs(N, random_state=rs)

    # p_trepo_sf: P(treponemal+ | serofast) — high but uncertain
    a, b = beta_ab(p_trepo_sf, lo=0.70, hi=0.999999)
    psa["p_trepo_sf"] = bd(a, b).rvs(N, random_state=rs)

    # p_ux_sf: P(unnecessary treatment | serofast detected) — high variability
    ux_lo = max(p_ux_sf - 0.15, 1e-6)
    ux_hi = min(p_ux_sf + 0.15, 0.60)
    ux_hi = max(ux_hi, ux_lo * 1.01)
    a, b = beta_ab(p_ux_sf, lo=ux_lo, hi=ux_hi)
...
def _deterministic_outcome_inputs() -> Tuple[dict, dict, dict]:
    """Return point-estimate background, untreated, and RR inputs for shared outcome calculations."""
...
def _incremental_screening_outcome_delta(n_screened, p_act, sens, p_adeq,
                                         tx_eff, prop_symp, prop_late):
    """
    Outcomes averted among the patients reached only because of the program.

    n_screened is already the incremental screened count after coverage and
    pregnancy-identification effects, so this compares no additional screen
    against one additional screen using the shared arm-count engine.
    """
    br_mean, ur_mean, rr_mean = _deterministic_outcome_inputs()
    no_screen = _arm_counts(0.0, p_act, 1.0, sens, p_adeq, tx_eff,
                            n_screened, br_mean, ur_mean, rr_mean,
                            prop_symp, prop_late)
    screen = _arm_counts(1.0, p_act, 1.0, sens, p_adeq, tx_eff,
                         n_screened, br_mean, ur_mean, rr_mean,
                         prop_symp, prop_late)
    return {k: no_screen[k] - screen[k] for k in no_screen}


def _arm_counts(
    sc, p_act, p_id, sens, p_adeq, p_tx, cohort,
    br, ur, rr, prop_symp, prop_late,
):
    """
    Arm outcome counts using the model's primary unconditional absolute-risk
    mixture.

    Untreated syphilis risks in UNT_ABS and PSA draws in ur are treated as
    absolute P(outcome | syphilis+ pregnancy). The intervention changes each
    absolute risk by the treatment relative risk through the effective treated
    fraction. This shared helper is used by PSA and deterministic helpers to
    avoid divergent outcome logic.
    """
    p_eff = sc * p_id * sens * p_adeq * p_tx

    def mix(unt, rr_key):
        tx = np.minimum(np.asarray(unt, dtype=float) * rr[rr_key], 1.0)
        return p_eff * tx + (1.0 - p_eff) * np.asarray(unt, dtype=float)
    ...
    misc_syph = np.asarray(ur["miscarriage"], dtype=float)  # no RR available
...
def _arm(sc, p_act, p_id, sens, p_adeq, p_tx, cohort,
    ...
    return _arm_counts(sc, p_act, p_id, sens, p_adeq, p_tx, cohort,
                       br, ur, rr, prop_symp, prop_late)
...
def _serofast_cost(n_screened, p_sf, p_trepo, p_ux, rpr, sf_wu, pen, soc_work, fta=None):
    """
    Incremental serofast module cost.

    Includes treponemal confirmation for serofast-positive screens, serofast
    workup, and unnecessary treatment when it occurs. The ``rpr`` argument is
    retained for backward-compatible call signatures; RPR screening itself is
    charged in the main screening-test cost.
    """
    n_sf = np.asarray(n_screened, dtype=float) * p_sf * p_trepo
    fta_cost = 0.0 if fta is None else np.asarray(fta, dtype=float)
    return n_sf * (fta_cost + sf_wu + p_ux * (pen + soc_work))


def _icost(d, cs, sf_cost, sc_b, sc_e, p_act, p_sf, p_trepo_sf, p_id,
           sens, spec, p_adeq, tx_eff,
           treat_fp, cohort,
           mk_lt_cost_saving,
    ...
    p_trepo_arr = np.asarray(p_trepo_sf, dtype=float)
    p_sn = np.maximum(1.0 - p_act - p_sf_arr, 0.0)
    p_fp = p_sn * (1.0 - spec)

    # n_tx: patients who complete treatment (generates both cost AND benefit)
    n_tx = extra * p_act * sens * p_adeq * tx_eff
    if treat_fp:
        n_tx = n_tx + extra * p_fp * p_adeq * tx_eff
...
def run_psa(
    ...
    ic_hs, ic_soc = _icost(dlt, cs, sf, sc_uc, sc_e, p_act,
                            psa_sf["p_sf"], psa_sf["p_trepo_sf"], p_id,
                            sens, spec, p_adeq, tx_eff,
    ...
    df["d_cs_total"] = (dlt["cs_comp"] + dlt["cs_uncomp"]).astype(float)
    df["d_sb_nnd_total"] = (dlt["stillbirth"] + dlt["neonatal_death"]).astype(float)
    ...
    smry = {
        ...
        **{f"d_{k}": summarize(v) for k, v in dlt.items()},
        "d_cs_total": summarize(dlt["cs_comp"] + dlt["cs_uncomp"]),
        "d_sb_nnd_total": summarize(dlt["stillbirth"] + dlt["neonatal_death"]),
...
def _det_icost(
    ...
    inc_preterm_yld: bool = True,
    tx_eff_override: float = None,
    treat_fp: bool = False,
) -> Tuple[float, float, float, float]:
    """
    Deterministic means for threshold/OWSA calculations.

    Returns (ic_hs, dal_hs, ic_soc, dal_soc). Outcome counts use the same
    unconditional absolute-risk mixture as the PSA engine, and sc_b is honored
    as the baseline screening coverage.
    """
    sc_uc = float(sc_b)
    _, tx_eff_default, _ = ges_eff()
    tx_eff = float(tx_eff_default if tx_eff_override is None else tx_eff_override)
    co = Costs()
    ...
    c_arm = _arm_counts(sc_uc, p_act, p_id, sens, p_adeq, tx_eff, cohort,
                        br_mean, ur_mean, rr_mean, prop_symp, prop_late)
    i_arm = _arm_counts(sc_e, p_act, p_id, sens, p_adeq, tx_eff, cohort,
                        br_mean, ur_mean, rr_mean, prop_symp, prop_late)
    dlt = {k: float(c_arm[k] - i_arm[k]) for k in c_arm}
    ...
    def _det_mk(p_h, p_m, p_s, ltc_obj=None):
        S = np.array([p_h, p_m, p_s, 0.0], dtype=float)
        da = 0.0
        cc = 0.0
        for t in range(T_):
            disc = (1.0 + r) ** (-t)
            ...
            mx_m = min(mu_x_m, max(1.0 - mu_t - q, 0.0))
            mx_s = min(mu_x_s, max(1.0 - mu_t, 0.0))
            excess_deaths = S[1] * mx_m + S[2] * mx_s
            ...
            S_new[0] += S[0] * (1.0 - mu_t)
            S_new[3] += S[0] * mu_t
            rm = max(1.0 - mu_t - q - mx_m, 0.0)
            S_new[1] += S[1] * rm
            S_new[2] += S[1] * q
            S_new[3] += S[1] * (mu_t + mx_m)
            rs = max(1.0 - mu_t - mx_s, 0.0)
            S_new[2] += S[2] * rs
            S_new[3] += S[2] * (mu_t + mx_s)
            S_new[3] += S[3]
            S = S_new
    ...
    dpc, cpc = _det_mk(max(1.0 - p_sev - p_mc, 0.0), p_mc, p_sev, ltc_obj=ltc)
    dpu, cpu = _det_mk(max(1.0 - p_mu, 0.0), p_mu, 0.0, ltc_obj=ltc)
    ...
    n_tx = extra * p_act * sens * p_adeq * tx_eff
    if treat_fp:
        n_tx += extra * p_fp * p_adeq * tx_eff
    test = (extra * co.poc + extra * co.rpr
            + extra * (p_act * sens + p_fp) * co.fta)
    sf_cost = _serofast_cost(extra, p_sf, p_trepo_sf, p_ux_sf,
                             co.rpr, co.sf_wu, co.pen, co.soc_work, fta=co.fta)
    prog = test + n_tx * (co.pen + co.soc_work) + extra * co.staff + sf_cost

    n_iufd = dlt["iufd_subset"]
    n_sb_t = max(dlt["stillbirth"] - n_iufd, 0.0)
...
def budget_impact_table(
    ...
    """Legacy deterministic BIA helper retained for backwards compatibility.

    The Streamlit BIA tab uses run_bia_scenario(); this helper now honors
    sc_b, treatment-completion overrides, false-positive treatment, and
    serofast confirmatory/workup/treatment costs.
    """
    co = Costs()
    sc_uc = float(sc_b)
    _, tx_eff_default, _ = ges_eff()
    ...
    n_tx_tp = extra * p_act * sens * p_adeq * tx_eff
    n_tx_fp = extra * p_fp * p_adeq * tx_eff if treat_fp else 0.0
    n_tx_yr = n_tx_tp + n_tx_fp
    sf_yr = _serofast_cost(
        extra, p_sf, p_trepo_sf, p_ux_sf, co.rpr, co.sf_wu, co.pen, co.soc_work, fta=co.fta
    )
    prog_yr = (extra * co.poc + extra * co.rpr
               ...
               + extra * co.staff
               + sf_yr)
    p_eff_d = max(sc_e - sc_uc, 0.0) * p_id * sens * p_adeq * tx_eff
...
def nmb_surface(prev_grid, tx_grid, p_sf, p_id, sc_b, sc_e, sens, spec,
                ...
                inc_mat_hosp_yld=True, inc_preterm_yld=True,
                tx_eff_override=None, treat_fp=False) -> np.ndarray:
...
def _fmt_param_value(x):
    """Compact display for scalar or linked multi-parameter OWSA values."""
    if isinstance(x, dict):
        return "; ".join(f"{k}={v:.3g}" if isinstance(v, (int, float, np.number)) else f"{k}={v}"
                         for k, v in x.items())
...
def _owsa_value_label(kw: dict):
    if len(kw) == 1:
        return _fmt_param_value(next(iter(kw.values())))
    return _fmt_param_value(kw)
...
def owsa_table(
    ...
    for label, (lo_kw, hi_kw) in param_ranges.items():
        ...
        rows.append({
            ...
            "Low param value": _owsa_value_label(lo_kw),
            "High param value": _owsa_value_label(hi_kw),
        })
...
def fig_ce_plane(dal, inc, title, wtp_lines=(50_000, 100_000, 150_000), subtitle=None):
    ...
    subtitle_text = subtitle if subtitle is not None else globals().get("preset", "")
    if subtitle_text:
...
def fig_tornado(owsa_df: pd.DataFrame, base_icer: float, top_n: int = 12):
    ...
    for i, row in df.iterrows():
        ...
        ax.text(row["ICER (high)"] + abs(base_icer)*0.005, i,
                _fmt_param_value(row['High param value']), va="center", fontsize=7, color="#4a90d9")
        ax.text(row["ICER (low)"]  - abs(base_icer)*0.005, i,
                _fmt_param_value(row['Low param value']), va="center", fontsize=7,
...
k6.metric("VSL net benefit (mean)",f"${smry['vsl_nb']['mean']/1e6:,.1f}M",
          help="Lives saved × VSL minus net health-sector incremental cost. "
...
with tabs[0]:
    ...
    ci4.metric(
        ...
        f"{smry['d_cs_total']['mean']:,.1f}",
        help=(
            f"95% CrI: "
            f"{smry['d_cs_total']['95% CrI lo']:,.1f}"
            f"–"
            f"{smry['d_cs_total']['95% CrI hi']:,.1f}"
        )
    )
    ci5.metric(
        "Stillbirths + NNDs averted (mean)",
        f"{smry['d_sb_nnd_total']['mean']:,.1f}",
        help=(
            f"95% CrI: "
            f"{smry['d_sb_nnd_total']['95% CrI lo']:,.1f}"
            f"–"
            f"{smry['d_sb_nnd_total']['95% CrI hi']:,.1f}"
        )
    )
    ...
    with st.expander("Outcomes averted — full breakdown with 95% CrI", expanded=False):
        ...
        for label, key in [
            ...
            ("Stillbirths + neonatal deaths", "d_sb_nnd_total"),
        ]:
            total_s = smry[key]
            lo = total_s["95% CrI lo"]
            hi = total_s["95% CrI hi"]
            mn = total_s["mean"]
            impact_rows.append({
                "Outcome": label,
                "Mean": f"{mn:,.1f}",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    param_ranges = {
        "Prevalence":          (prevalence_low, prevalence_high),
        "Treatment rate":      ({"p_adeq": 0.50}, {"p_adeq": 0.95}),
        "P(pregnancy ID)":     ({"p_id": 0.65},   {"p_id": 0.98}),
        serofast_label:         (serofast_low, serofast_high),
...
with tabs[1]:
    ...
    with st.expander("📌 VSL Net-Benefit Analysis (separate framework — not an ICER)", expanded=True):
        ...
        > as: :red[*"Does the monetary value society places on lives saved exceed the net health-sector incremental cost?"*]
        ...
        st.caption(
            ...
            f"Net benefit = VSL × (stillbirths + neonatal deaths prevented) − net health-sector incremental cost."
        )
...
with tabs[2]:
    ...
    diag_cas = bia_screening_cascade(
        n_screened = n_incr_yr1,
        p_act      = p_act,
        p_sf       = p_sf,
        sens       = sens,
        spec       = spec,
    ...
    n_tp_det_yr1  = diag_cas["n_tp_detected"]
    n_tp_tx_yr1   = diag_cas["n_tp_treated"]
    n_sf_det_yr1  = diag_cas["n_sf_detected"]
    n_fp_det_yr1  = diag_cas["n_fp_detected"]

    # Step 5 — outcomes averted (year 1) from the shared outcome engine
    diag_dlt = _incremental_screening_outcome_delta(
        n_screened = n_incr_yr1,
        p_act      = p_act,
        sens       = sens,
        p_adeq     = p_adeq,
    ...
    n_cs_av_yr1  = float(diag_dlt["cs_comp"] + diag_dlt["cs_uncomp"])
    n_sb_av_yr1  = float(diag_dlt["stillbirth"])
    n_nnd_av_yr1 = float(diag_dlt["neonatal_death"])

    # Step 6 — quick cost sanity check (year 1, point estimates)
    co_d = Costs()
    cost_tests_yr1 = diag_cas["n_screened"] * (co_d.poc + co_d.rpr)
    cost_conf_yr1  = diag_cas["n_confirmatory"] * co_d.fta
    cost_tx_tp_yr1 = diag_cas["n_tp_treated"] * (co_d.pen + co_d.soc_work)
    cost_tx_sf_yr1 = diag_cas["n_sf_treated"] * (co_d.pen + co_d.soc_work)
    cost_tx_fp_yr1 = diag_cas["n_fp_treated"] * (co_d.pen + co_d.soc_work)
    cost_tx_yr1    = cost_tx_tp_yr1 + cost_tx_sf_yr1 + cost_tx_fp_yr1
    cost_sf_wu_yr1 = diag_cas["n_sf_detected"] * co_d.sf_wu
    cost_staff_yr1 = diag_cas["n_screened"] * co_d.staff
    cost_total_yr1 = (cost_tests_yr1 + cost_conf_yr1 + cost_tx_yr1
                      + cost_sf_wu_yr1 + cost_staff_yr1)
    ...
    diag_rows = [
        ...
        ("Treatment — active TP",                  f"${cost_tx_tp_yr1:,.0f}", "Derived",""),
        ("Treatment — serofast",                   f"${cost_tx_sf_yr1:,.0f}", "Derived",""),
        ("Treatment — false positive",             f"${cost_tx_fp_yr1:,.0f}", "Derived",""),
        ("Serofast workup",                        f"${cost_sf_wu_yr1:,.0f}", "Derived",""),
        ("Staff / workflow",                       f"${cost_staff_yr1:,.0f}", "Derived",""),
        ("Total program cost — Year 1",            f"${cost_total_yr1:,.0f}", "Derived",
             "← should match the BIA table's program cost components"),
    ]
...
with tabs[3]:
    ...
    with st.spinner("Computing NMB surface…"):
        ...
        G = nmb_surface(prev_g, tx_g, p_sf, p_id, sc_uc_eff, sc_e, sens, spec,
                        ...
                        inc_misc_yld=inc_misc_yld, inc_mat_hosp_yld=inc_mat_hosp_yld,
                        inc_preterm_yld=inc_preterm_yld,
                        tx_eff_override=tx_eff, treat_fp=treat_fp)
...
with tabs[5]:
    ...
    s3.metric("Mean serofast module cost", f"${sf_mean:,.0f}")
    ...
    ax_sf.set_xlabel("Serofast module cost per cohort"); ax_sf.set_ylabel("Frequency")
    ax_sf.set_title("PSA Distribution: Serofast Module Cost", fontweight="bold")
    ...
    | Component | Value |
    |-----------|-------|
    | Serofast confirmation, workup & unnecessary treatment | **${sf_mean:,.0f}** |
...
with tabs[6]:
    ...
    with st.expander("Outcome risk mixture — derivation", expanded=False):
        st.markdown("""
        **How outcome risks are applied (reviewer note)**

        The PSA engine and deterministic helpers now use the same
        **unconditional absolute-risk mixture**.

        1. Untreated syphilis risks are interpreted as absolute
           `P(outcome | syphilis+ pregnancy)` values.
        2. For each outcome with an available treatment relative risk, treatment
           changes that absolute risk by `RR` among the effectively treated
           fraction.
        3. The arm-level syphilis risk is therefore
           `p_eff × untreated risk × RR + (1 − p_eff) × untreated risk`.
        4. Background risks are mixed into stillbirth, neonatal death, preterm,
           LBW, and miscarriage. Congenital syphilis is treated as a
           syphilis-specific outcome with no background rate.

        This intentionally avoids using one outcome hierarchy for PSA and a
        different hierarchy for OWSA/threshold analysis. The published absolute
        risks are not reinterpreted as conditional probabilities inside the
